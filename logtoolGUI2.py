...
def visualize_logins():
    ...
    try:
        ...
        plt.bar([i - bar_width/2 for i in x], failed_values, width=bar_width, label='Failed Logins', color='#E74C3C', align='center') # Darker Red
        plt.bar([i + bar_width/2 for i in x], success_values, width=bar_width, label='Successful Logins', color='#27AE60', align='center') # Darker Green
...
# --- GUI Definition (from code3GUI.py, with original names and new improved look) ---

def create_gui():
    root = tk.Tk()
    root.title("Designing a System for Detecting Suspicious Network Activities") # More engaging title
    root.geometry("550x650") # Increased size for better layout
    root.configure(bg="#2C3E50") # Deep blue-gray, professional and modern

    # --- Header ---
    header = tk.Label(
        root,
        text=" Cyber Security Log Analyzer ", # Added shield emojis for visual appeal
        font=("Fira Code", 22, "bold"), # More modern, 'coding' font, bold
        bg="#2C3E50",
        fg="#ECF0F1"  # Light gray for contrast
    )
    header.pack(pady=40) # Increased padding
    ...
    # Enhanced button styling for a modern, distinct look
    button_style = {
        "width": 35, # Wider buttons
        "height": 2, # Taller buttons
        "bg": "#3498DB",        # Vibrant blue for main action buttons
        "fg": "#FFFFFF",        # White text
        "activebackground": "#2980B9", # Darker blue on press
        "relief": "flat",       # Flat design, modern
        "bd": 0,                # No border
        "font": ("Fira Code", 13, "bold"), # Larger, bold, modern font
        "cursor": "hand2"       # Hand cursor on hover
    }

    buttons_config = [
        {"text": "Extract Auth Logs", "command": on_extract_logs}, # More descriptive text
        {"text": "Analyze Login Attempts", "command": on_analyze_logins}, # More descriptive text
        {"text": "Visualize Login Patterns", "command": on_visualize_data}, # More descriptive text
        {"text": "Analyze Web Server Logs", "command": on_analyze_iis_logs} # More descriptive and general text
    ]

    for config in buttons_config:
        btn = tk.Button(button_frame, text=config["text"], command=config["command"], **button_style)
        btn.pack(pady=12) # Increased padding between buttons for breathing room

    # --- Footer/Instructions ---
    footer = tk.Label(
        root,
        text="Ensure sudo privileges for log access operations.", # Concise instruction
        font=("Arial", 11, "italic"),
        bg="#2C3E50",
        fg="#BDC3C7" # Lighter gray for instructional text
    )
    footer.pack(pady=30)
